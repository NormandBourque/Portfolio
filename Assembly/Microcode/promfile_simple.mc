...
81:alu := tir + tir; if n then goto 89;		{ 111 1111 01 = RSHIFT}
...
{RSHIFT <ARGUMENT IS VALUE 0-15> Right shift the current accumulator by the argument number of bits.  This is a zero-fill operation and the argument must be a value between 0 and 15(a four bit magnitude only value, located in the least significant 4 bits)}

{MULTM <ARGUMENT IS A VALUE 0-63> Multiply the value at the top of the stack by the six bit immediate value in the instruction(remember, we're using 10 bit op codes here). The result of the multiplication should replace the value at the top of the stack, provided that no overflow occurred.  If the multiplication would overflow a 16 bit result, the top of the stack must not be changed.  After the operation, the accumulator must be set to 0 if the multiplication succeeded, and -1 if an overflow occurred }

{DIV <NO ARGUMENTS> divide the 16 bit 2s complement number at the top stack location sp(the dividend) by 15 bit 2s complement number at the location just under the top location SP+1(the divisor), and push on two new locations with SP-1 having the (unsigned) remainder of the division and SP-2(new top of stack) having the 16 bit 2s complement quotient.  If the absolute value of the divisor is greater than the absolute value of the dividend, the quotient should be set to 0 and the (unsigned) remainder set to the absolute value of the dividend, and if the divisor is equal to 0(this is an illegal case) the remainder should be set to -1 and the quotient should be set to 0. After the operation, the accumulator must be set to 0 if the division was legal, and -1 if an attempt to divide by 0}
