...
void genamux(bool set)
{
	...
	if (word[0] < 0)
		...
		printerr(semanticerr, reset, (long) "amux");
// long cast used for string address on 64 bit systems  wm_9_18
// also use with other printerr calls in this file
	word[20] = false;
}
...
void gencond(int c)
{
	...
	if (word[1] < 0)
		...
		printerr(semanticerr, reset, (long) "jump condition");
}
...
void genalu(int f)
{
	...
	if (word[3] < 0)
		...
		printerr(semanticerr, reset, (long) "alu function");
}
...
void genshift(int f)
{
	...
	if (word[5] < 0)
		...
		printerr(semanticerr, reset, (long) "shift function");
}
...
void genmbr(bool set)
{
	...
	if (word[7] < 0)
		...
		printerr(semanticerr, reset, (long) "mbr");
}
...
void genmar(bool set)
{
	...
	if (word[8] < 0)
		...
		printerr(semanticerr, reset, (long) "write");
}
...
void genread(bool set)
{
	...
	if (word[9] < 0)
		...
		printerr(semanticerr, reset, (long) "read");
}
...
void genenc(bool set)
{
	...
	if (word[11] < 0)
		...
		printerr(semanticerr, reset, (long) "enc");
}
	...
	else
	{
		...
		if (word[12] == -1)
			...
			if (!isequalb(temparray, word+12, 4))
				printerr(semanticerr, reset, (long) "C bus");
	}
}
	...
	else
	{
		...
		if (word[16] == -1)
			...
			if (!isequalb(temparray, word+16, 4))
				printerr(semanticerr, reset, (long) "B bus");
	}
}
	...
	else
	{
		...
		if (word[20] == -1)
			...
			if (!isequalb(temparray, word+20, 4))
				printerr(semanticerr, reset, (long) "A bus");
	}
}
	...
	if (panicmode) return;
	{
		...
		if (word[24] == -1)
			...
			if (!isequalb(temparray, word+24, 8))
				printerr(semanticerr, reset, (long) "jump address");
	}
}
...
void genabreg(enreg dreg)
{
	...
	else
	{
		...
		if (isequalb(temparray, word+20, 4))
			...
			printerr(semanticerr, reset, (long) "A/B bus");
	}
}
