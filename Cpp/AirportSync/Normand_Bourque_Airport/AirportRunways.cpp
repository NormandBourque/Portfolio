...
void AirportRunways::checkAirportStatus(RunwayNumber requestedRunway)
{
	...
	for (int i = RUNWAY_4L; i <= RUNWAY_15R; i++)
	{
		...
		if (runwayInUse[i] > 1)
		{
			cout << "***** The nu4R...ber of planes landing on runway " << runwayName(RunwayNumber(i)) << " is greater than 1!\n";
			crash = true;
		}
	}

	// Check individual restrictions on each runway
	if ((runwayInUse[RUNWAY_9] > 0) && ((runwayInUse[RUNWAY_4R] > 0) || (runwayInUse[RUNWAY_15R] > 0)))
	{
		cout << "***** Runways 9, 4R, and/or 15R may not be used simultaneously!\n";
		crash = true;
	}

	if (((runwayInUse[RUNWAY_15L] > 0) || (runwayInUse[RUNWAY_15R] > 0)) && ((runwayInUse[RUNWAY_4L] > 0) || (runwayInUse[RUNWAY_4R] > 0)))
	{
