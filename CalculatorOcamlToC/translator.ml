...
let rec ast_ize_P (p:parse_tree) : ast_sl =
  match p with
	| PT_nt ("P", [statement_list; PT_term "$$"]) -> (ast_ize_SL statement_list)
...
and ast_ize_expr_tail (lhs:ast_e) (tail:parse_tree) : ast_e =
  (* lhs is an inherited attribute.
	...
	| PT_nt ("TT", []) -> lhs
	| PT_nt ("FT", []) -> lhs
	| PT_nt ("TT", [PT_nt (ao ,[PT_term operator]); term; term_tail]) -> AST_binop (operator, lhs, (ast_ize_expr_tail(ast_ize_expr(term)) term_tail))
	| PT_nt ("FT", [PT_nt (mo, [PT_term operator]); factor; factor_tail]) -> AST_binop (operator, lhs, (ast_ize_expr_tail(ast_ize_expr(factor)) factor_tail) )
...
let message, output = translate (ast_ize_P (parse ecg_parse_table primes_prog))in
